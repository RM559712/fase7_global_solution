    ...
    def send_message_by_alert_humidity(self, str_sensor_name: str = None, str_location_name: str = None, float_location_max_humidity: float = 0.00, float_measurement_value: float = 0.00, str_insert_date: str = None)-> dict:
        ...
        try:
            ...
            dict_params_request['str_message'] = f'''
                ...
                Uma medição foi cadastrada a partir do sensor "{str_sensor_name}" na localização "{str_location_name}" com o valor de {float_measurement_value} ml em {str_insert_date}.

                De acordo com a configuração dessa localização, a medição é superior ao máximo permitido ( {float_location_max_humidity} ml ), podendo ocasionar as possíveis situações:
